...
runtimes_.drop_duplicates(subset=["dataset"], inplace=True)
...
runtimes_large_msas["speedup_to_sbs"] = (runtimes_large_msas['elapsed_time_sbs'] / runtimes_large_msas['elapsed_time_ebg'])
avg_speedup = runtimes_large_msas["speedup_to_sbs"].mean()
std_speedup = runtimes_large_msas["speedup_to_sbs"].std()
print(f"Average speedup of EBG + Inference to SBS: {avg_speedup}, Std.: {std_speedup}")
print("#" * 100)
...
plt.plot(sorted_time_merged["msa_size_patterns"], sorted_time_merged["moving_avg_ebg"], label='EBG', color='gray',
         linestyle="dotted")

plt.plot(sorted_time_merged["msa_size_patterns"], sorted_time_merged["moving_avg_sbs"], label='SBS', color='red', linestyle="dashdot")
...
plt.legend()
plt.tight_layout()
plt.savefig("paper_figures/runtimes_high.png", dpi=900)
sys.exit()
...
sorted_time_merged = runtimes.sort_values(by='num_seq')
...
plt.plot(sorted_time_merged[sorted_time_merged['isDNA'] == 1]['num_seq'],
         sorted_time_merged[sorted_time_merged['isDNA'] == 1]['moving_avg_iqtree_1'], label='UFBoot2 (DNA)',
         color='blue')
plt.plot(sorted_time_merged[sorted_time_merged['isDNA'] == 0]['num_seq'],
         sorted_time_merged[sorted_time_merged['isDNA'] == 0]['moving_avg_iqtree_0'], label='UFBoot2 (AA)',
...
plt.plot(sorted_time_merged["num_seq"], sorted_time_merged["moving_avg_ebg"], label='EBG', color='green',
         linestyle="dotted")

plt.plot(sorted_time_merged["num_seq"], sorted_time_merged["moving_avg_sbs"], label='SBS', color='red')
...
plt.legend()
plt.tight_layout()
plt.savefig("paper_figures/runtimes_num_seq.png")
